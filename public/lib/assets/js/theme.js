		...
		initBxSlider: function(options) {
			...
			if(bxSliderBox && bxSliderBox.length){
				bxSliderBox.each(function(i) {
					var $bx = $(this);
					var pagerCustomData = $bx.data('pager-custom') || "#bx-pager"
					var controlsData = $bx.data('controls') || "false"
					var minSlidesData = $bx.data('min-slides') || "1"
					var maxSlidesData = $bx.data('max-slides') || "1"
					var slideWidthData = $bx.data('slide-width') || "0"
					var slideMarginData = $bx.data('slide-margin') || "0"
					var pagerData = $bx.data('pager') || "true"
					var modeData = $bx.data('mode') || "horizontal"
					var infiniteLoopData = $bx.data('infinite-loop') || "true"
					var prevText = (modeData == 'vertical' ? '<span class="fa fa-angle-up"></span>' : '<span class="fa fa-angle-left"></span>');
					var nextText = (modeData == 'vertical' ? '<span class="fa fa-angle-down"></span>' : '<span class="fa fa-angle-right"></span>');
